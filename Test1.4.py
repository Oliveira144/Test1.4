...
# ================= CONFIGURAÇÃO =================
st.set_page_config(
    page_title="Football Studio AI – Completo + Atualizado",
    layout="wide"
)

MAX_HISTORY = 90        # 9 colunas x 10 linhas
WINDOW_ALT = 6          # janela mínima alternância
WINDOW_PATTERN = 12     # leitura de padrão
...
# ================= LÓGICA ======================
def detect_alternance(history, window=WINDOW_ALT):
    if len(history) < window:
        return {"status": "Insuficiente", "confidence": 0}
...
def detect_pattern(history, window=WINDOW_PATTERN):
    if len(history) < window:
...
def detect_manipulation_level(history):
    alt = detect_alternance(history)
    rep = detect_repetition(history)
    ...
    if alt["status"] == "Falsa Alternância":
        return 7
    if pattern.startswith("Padrão misto"):
        return 8

    return 5


def detect_breach(history):
    if len(history) < 7:
        return False

    alt = detect_alternance(history)
    return alt["status"] == "Alternância Limpa" and alt["confidence"] > 80


def predict_next(history):
    if len(history) < 3:
        return "Sem previsão", 0
...
manip_level = detect_manipulation_level(hist)
breach = detect_breach(hist)
pred, conf = predict_next(hist)
...
with colA:
    st.metric("Alternância", alt["status"])
with colB:
    st.metric("Confiança", f"{alt['confidence']}%")
with colC:
    st.metric("Repetição", rep)
with colD:
    st.metric("Nível Manipulação", manip_level)
with colE:
    st.metric("Brecha", "SIM" if breach else "NÃO")

st.subheader("🧩 Leitura do Cenário")
st.info(pattern)

st.subheader("🎯 Previsão / Ação")
if breach and conf >= 70:
    st.success(f"BRECHA DETECTADA → Entrada sugerida: {pred} ({conf}%)")
elif conf >= 60:
    st.warning(f"Possível caminho: {pred} ({conf}%)")
else:
    st.info("Momento de espera – padrão instável")

st.caption(
    "Sistema completo: histórico 9x10, alternância relacional, repetição, "
    "leitura de padrão, brechas, previsão e níveis de manipulação"
    )
