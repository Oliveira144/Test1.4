...
def detect_pattern(history, window=WINDOW_PATTERN): if len(history) < window: return "Dados insuficientes" seq = list(history)[-window:] counts = Counter(seq) dominant = counts.most_common(1)[0] if dominant[1] / window > 0.6: return f"Tendência dominante {dominant[0]}" return "Padrão misto / camuflado"
